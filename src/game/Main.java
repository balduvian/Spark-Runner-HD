...
public class Main extends CNGE {
	...
	protected Main() {
		TexturePreset.setDefaults(GL_REPEAT, GL_REPEAT, GL_NEAREST, GL_NEAREST);

		initGameSize(256, 144);
		initScreenMode(Screen.makeAspectScreen(), -1);
		initWindow(new Window().initFull(false).initName("Spark Runner HD").initIcon("res/images/icon.png").init());
